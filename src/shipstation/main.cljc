(ns shipstation.main
  "Kotodama WASM entrypoint for the Shipstation clean-room actor (L4) — Clojure port.
  ...
  facts (`shipstation.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shipstation")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"shipstation.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shipstation-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shipstation-compat/ready)
